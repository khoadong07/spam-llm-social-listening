"""
GHN Spam Filter v10 — bộ lọc data rác cho dự án Giao Hàng Nhanh (GHN).

Lịch sử phiên bản:
  v7 (08/05/2026): keyword cơ bản theo file keyword_rac_GHN.xlsx
  v8 (10/05/2026): thắt chặt brand pattern (tránh false-positive khi tên công ty
                    spam đứng cách "giao hàng nhanh" rất xa); thêm pattern gas,
                    khô cá, tấm ốp nano, băng quấn, phở cuốn; bổ sung
                    phone-dotted (0978.21.06.17); thêm "giao hàng nhanh +
                    sales signal mà không có brand GHN" → Rác.
  v9 (11/05/2026): Brand-Discussion Safe-Harbor — bài thảo luận/khiếu nại/so
                    sánh các đơn vị vận chuyển (GHN, GHTK, SPX, J&T, Viettel
                    Post...) không phải spam; brand-strong safe-harbor được
                    di chuyển lên trước check spam content để bài chính thức
                    của GHN không bị nhầm thành rác chỉ vì content mention
                    "shopee.vn".
  v10 (11/05/2026): thêm patterns mới user yêu cầu — gửi hàng đi Mỹ/Úc/quốc
                     tế, nệm cao su/PU, đại lý nước uống, đấu giá với giá khởi
                     điểm, phớt thủy lực, mực khô siêu to dày chất, bàn ghế ăn
                     nhập khẩu (bàn đá ceramic, ghế da xương cá), hỏa tốc 247.

Cơ chế: GIỮ data, thêm cột "Phân loại" = "Rác" / "Không rác".
Tùy chọn `--delete` để xóa hẳn dòng rác.

Cách dùng:
    python ghn_spam_filter_v10.py <input.xlsx>            # giữ + highlight đỏ
    python ghn_spam_filter_v10.py <input.xlsx> --delete   # xóa hẳn dòng rác
"""
import pandas as pd
import re
...
# ============== Brand strong (đã thắt chặt ở v8) ==============
# Pattern cũ "công ty.*giao hàng nhanh" dùng .* greedy → match false-positive
# khi tên công ty spam đứng cách "giao hàng nhanh" rất xa. v8 chuyển sang
# pattern liền kề: "công ty (cổ phần/tnhh/cp) giao hàng nhanh".
GHN_BRAND_STRONG = [
...
PRODUCT_CATEGORIES = (
    ...
    # v8: gas, khô cá, tấm ốp, băng quấn, phở cuốn
    ...
    # v10: gửi hàng quốc tế, nệm, giao nước, đấu giá, phớt, mực khô, bàn ghế ăn, hỏa tốc 247
    ...
    r'|dịch vụ chuyển hàng quốc tế|chuyển hàng quốc tế'
...
# v8: thêm phone dotted (0978.21.06.17, 0932.186.107)
CONTACT_PATTERNS = (
...
SPAM_CONTENT_PATTERNS = [
    ...
    r'cnc.*nan xoan',
    # v8: gas, khô cá, tấm ốp, băng quấn, phở cuốn + competitor + giao hàng nhanh
    ...
    r'cá lóc.*1 nắng',
    r'phở cuốn.*giao',
    # v10 NEW: gửi hàng quốc tế, nệm, giao nước, đấu giá, phớt, mực khô, bàn ghế ăn, hỏa tốc 247
...
# ============== v9: Brand-discussion / multi-carrier safe-harbor ==============
# Bài thảo luận/khiếu nại/so sánh các đơn vị vận chuyển (GHN, GHTK, SPX,
# J&T, Viettel Post...) → buzz về thương hiệu GHN, không phải spam.
...
def _compile():
    """Compile các regex 1 lần để chạy nhanh hơn."""
    return {
...
def _count_carriers(full):
    """Đếm số đơn vị vận chuyển distinct được nhắc đến trong text."""
    found = set()
    for m in _REGEX['carrier_list'].finditer(full):
...
def classify_ghn_spam(row, title_col='Title', content_col='Content',
                      desc_col='Description', site_col='SiteName'):
    """Phân loại 1 dòng → 'Rác' hoặc 'Không rác'."""
    parts = []
    for c in [title_col, content_col, desc_col, site_col]:
        if c is None:
            continue
        v = row.get(c, '')
        if pd.notna(v):
    ...
    site = str(row.get(site_col, '') or '').lower() if site_col else ''

    # L0: whitelist site (cộng đồng GHN)
    ...
    if has_price: score += 1
    if has_contact: score += 1
    if has_product: score += 2
    if has_cta: score += 2

    # v9: Brand-Discussion Safe-Harbor (kích hoạt khi score < 5)
    # Bài thảo luận/khiếu nại/so sánh đơn vị vận chuyển → giữ Không rác.
    if score < 5:
    ...
    # L3: spam content
    if _REGEX['spam_content'].search(full):
        return 'Rác'

    # L3.5 (v8): "giao hàng nhanh" + sales signal mà không có brand → Rác
    ...
    # L7: ship CTA mạnh
    if _REGEX['ship_cta'].search(full):
        return 'Rác'
...
def detect_columns(df):
    """Tự dò cột Title/Content/Description/SiteName theo tên hoặc vị trí."""
    cols = {c.lower().strip(): c for c in df.columns}

    def find(*names, fallback_idx=None):
        for n in names:
            if n.lower() in cols:
                return cols[n.lower()]
    ...
    return {
        'title':   find('title', 'tiêu đề', fallback_idx=3),
        'content': find('content', 'nội dung', fallback_idx=4),
        'desc':    find('description', 'mô tả', fallback_idx=5),
        'site':    find('sitename', 'site name', 'site', fallback_idx=9),
    }


def process_ghn_file(input_path, output_path=None, delete_spam=False):
    """Mặc định: GIỮ data + thêm cột "Phân loại" + tô đỏ dòng rác.
       delete_spam=True: xóa hẳn dòng rác."""
    if output_path is None:
        suffix = '_clean.xlsx' if delete_spam else '_filtered.xlsx'
        output_path = input_path.replace('.xlsx', suffix)

    df = pd.read_excel(input_path)
    cols = detect_columns(df)
    print(f"Mapping cột: {cols}")

    df['Phân loại'] = df.apply(
        lambda r: classify_ghn_spam(
            r, cols['title'], cols['content'], cols['desc'], cols['site']
        ), axis=1
    )
    spam = (df['Phân loại'] == 'Rác').sum()
    total = len(df)
    pct = spam / total * 100 if total else 0
    print(f"Kết quả: {spam}/{total} dòng rác ({pct:.1f}%)")

    if delete_spam:
        out = df[df['Phân loại'] != 'Rác'].drop(columns=['Phân loại'])
        out.to_excel(output_path, index=False)
        print(f"[XÓA] Loại {spam} dòng → {output_path}")
        return out
    ...
    hdr = ws.cell(row=1, column=last_col)
    ...
    for i in range(2, total + 2):
        c = ws.cell(row=i, column=last_col)
        if c.value == 'Rác':
            for j in range(1, last_col + 1):
                ws.cell(row=i, column=j).fill = red_fill
            c.font = bold_red
    wb.save(output_path)
    print(f"[GIỮ] Highlight đỏ → {output_path}")
    return df
...
if __name__ == '__main__':
    import sys
    if len(sys.argv) < 2:
        print("Usage: python ghn_spam_filter_v10.py <input.xlsx> [--delete]")
        sys.exit(1)
    delete = '--delete' in sys.argv
    process_ghn_file(sys.argv[1], delete_spam=delete)
